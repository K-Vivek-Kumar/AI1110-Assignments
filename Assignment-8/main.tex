...
\vspace{3cm}
	\section{Papoulis-Chapter-10} \textbf{\underline{Question 10-17:}} Find the power spectrum $S\brak{\omega}$ of a process $x(t)$ if $S\brak{\omega}=0$ for $|\omega|>\pi$ and
	\begin{align*}
	E\cbrak{x(n+m)x(n)}=N\delta\sbrak{m}
	\end{align*}
	\par\textbf{\underline{Solution:} }
	Given,
	\begin{align}
	E\cbrak{x(n+m)x(n)}=N\delta\sbrak{m}
	\end{align}
	As we know, with $T=\dfrac{\pi}{\sigma}$
	\begin{align}
	...
	\end{align}
	As it is said that $S\brak{\omega}=0$ for $|\omega|>\pi$, therefore we can say from the equation \ref{eqn-1.6}, $\eta=0$ and $I=N$. On substituting it in the equation,
	\begin{align}
